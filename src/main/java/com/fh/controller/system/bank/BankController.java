...
public class BankController extends BaseController {
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).get("BANK_ID").toString());	//1
			vpd.put("var2", varOList.get(i).getString("BANK_NAME"));	    //2
			vpd.put("var3", varOList.get(i).getString("BANK_URL"));	    //3
			vpd.put("var4", varOList.get(i).getString("INFO"));	    //4
			vpd.put("var5", varOList.get(i).get("CLICK_NUMBER").toString());	//5
			vpd.put("var6", varOList.get(i).getString("CREATE_TIME"));	    //6
			vpd.put("var7", varOList.get(i).getString("LOGO"));	    //7
			vpd.put("var8", varOList.get(i).get("APP_ID").toString());	//8
			vpd.put("var9", varOList.get(i).getString("BACKAGE_ID"));	    //9
			varList.add(vpd);
		}
