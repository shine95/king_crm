...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="loanuser/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">贷款用途</th>
									<th class="center">职业</th>
									<th class="center">所在地</th>
									<th class="center">房子</th>
									<th class="center">车辆</th>
									<th class="center">信用卡</th>
									<th class="center">公积金</th>
									<th class="center">微粒贷</th>
									<th class="center">保险</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.SYSTEM_TYPE}</td>
											<td class='center'>${var.CHANNEL}</td>
											<td class='center'>${var.LOAN_AMOUNT}</td>
											<td class='center'>${var.LOAN_TERM}</td>
											<td class='center'>${var.USER_NAME}</td>
											<td class='center'>${var.ID_CARD}</td>
											<td class='center'>${var.CURLTURE}</td>
											<td class='center'>${var.PURPOSE}</td>
											<td class='center'>${var.WORK}</td>
											<td class='center'>${var.ADDRESS}</td>
											<td class='center'>${var.HOUSE}</td>
											<td class='center'>${var.CAR}</td>
											<td class='center'>${var.CREDIT_CARD}</td>
											<td class='center'>${var.ACCUMULATION_FUND}</td>
											<td class='center'>${var.WECHAT_LOAN}</td>
											<td class='center'>${var.INSURANCE}</td>
											<td class='center'>${var.SOCIAL_SECURITY}</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>loanuser/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>loanuser/delete.do?USER_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>loanuser/goEdit.do?USER_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>loanuser/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>loanuser/excel.do';
		}
	</script>
